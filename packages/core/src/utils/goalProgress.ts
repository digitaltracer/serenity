 ...
 */
export function getCurrentPeriod(timeframe: 'daily' | 'weekly' | 'monthly', customStart?: Date): { start: Date; end: Date } {
 ...
 */
function calculateWeeklyTasksProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
 ...
 */
function calculateProjectTasksProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
  const { start, end } = getCurrentPeriod(goal.config.timeframe);
  const target = goal.config.targetCount || 0;
 ...
 */
function calculatePriorityTasksProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
  const { start, end } = getCurrentPeriod(goal.config.timeframe);
 ...
 */
function calculateDailyStreakProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
 ...
 */
function calculateJournalWeeklyProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
  const { start, end } = getCurrentPeriod(goal.config.timeframe);
  const target = goal.config.targetCount || 0;
 ...
 */
function calculateCompletionRateProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
  const { start, end } = getCurrentPeriod(goal.config.timeframe);
 ...
 */
export function calculateGoalProgress(goal: Goal, data: ProgressCalculationData): GoalProgress {
  switch (goal.type) {
    case 'weekly_tasks':
 ...
 */
export function getGoalDescription(goal: Goal, projects?: Project[]): string {
