...
export interface InsightQualityScore {
  relevance: number; // 0-1: How relevant to current user context
  actionability: number; // 0-1: How clear and actionable
  novelty: number; // 0-1: How new vs. repeated advice
  impact: number; // 0-1: Potential productivity gain
  confidence: number; // 0-1: AI confidence + data quality
...
export interface ScoredInsight extends AIInsight {
  ...
  similarTo?: string[]; // IDs of similar insights
  supersedes?: string; // ID of insight this replaces
}
...
export class InsightQualityService {
  ...
  private static readonly SIMILARITY_THRESHOLD = 0.75; // For deduplication
  ...
  private static calculateRelevanceScore(
    ...
    if (insight.type === 'warning') score += 0.2;
    if (insight.type === 'recommendation') score += 0.15;

    // Category relevance
    if (context?.focusAreas) {
      const categoryMatch = context.focusAreas.some(area =>
   ...
   */
  private static calculateActionabilityScore(insight: AIInsight): number {
    ...
    if (hasNumberedSteps) score += 0.15;
  ...
  private static calculateNoveltyScore(
    ...
    if (previousInsights.length === 0) return 1.0;

    // Compare title and description similarity
    const similarities = previousInsights.map(prev => 
   ...
   */
  private static calculateImpactScore(insight: AIInsight): number {
    let score = 0.5; // Base impact

    // Type-based impact
    if (insight.type === 'warning') score += 0.3; // Warnings often high impact
    if (insight.type === 'recommendation') score += 0.2;
    if (insight.type === 'productivity') score += 0.15;

    // Category-based impact
    if (insight.category === 'goals') score += 0.15;
    if (insight.category === 'tasks') score += 0.1;
    ...
    if (insight.metadata?.impact) {
      const impactValue = {
        ...
        medium: 0.15,
        low: 0.0,
      }[insight.metadata.impact as string] || 0;
      score += impactValue;
    }

    // Check for urgency indicators
    const urgentTerms = ['overdue', 'urgent', 'critical', 'immediately', 'now'];
    const hasUrgency = urgentTerms.some(term => 
   ...
   */
  private static calculateOverallScore(scores: Omit<InsightQualityScore, 'overall'>): number {
    const weights = {
      relevance: 0.25,
      actionability: 0.30,
      novelty: 0.20,
  ...
  static scoreInsight(
    ...
    const qualityScore: InsightQualityScore = {
      relevance,
      actionability,
      novelty,
      impact,
   ...
   */
  static validateInsight(insight: AIInsight): { valid: boolean; reasons: string[] } {
  ...
  static filterByQuality(
    ...
    minimumScore: number = this.QUALITY_THRESHOLDS.minimum
  ): ScoredInsight[] {
   ...
   */
  private static areSimilar(insight1: ScoredInsight, insight2: ScoredInsight): boolean {
    ...
    if (insight1.category === insight2.category && insight1.type === insight2.type) {
      const textSimilarity = this.calculateTextSimilarity(
        insight1.title + ' ' + insight1.description,
        insight2.title + ' ' + insight2.description
      );
  ...
  static deduplicateInsights(scoredInsights: ScoredInsight[]): DeduplicationResult {
    ...
    const uniqueInsights: ScoredInsight[] = [];

    for (const insight of scoredInsights) {
      if (processed.has(insight.id)) continue;

      // Find all similar insights
      const similarInsights = scoredInsights.filter(
        other => other.id !== insight.id && !processed.has(other.id) && this.areSimilar(insight, other)
      );
    ...
    logger.info(
      `✨ Deduplication: ${scoredInsights.length} insights → ${uniqueInsights.length} unique (removed ${scoredInsights.length - uniqueInsights.length} duplicates)`,
      { component: 'insightQualityService', operation: 'deduplicateInsights' }
    );
  ...
  static processInsights(
    ...
    logger.info(`🔍 Processing ${insights.length} raw insights through quality pipeline`, {
    ...
    const validatedInsights = insights.filter(insight => {
      ...
      if (!validation.valid) {
        logger.warn(`⚠️ Invalid insight rejected: ${validation.reasons.join(', ')}`, {
          component: 'insightQualityService',
          operation: 'validate',
    ...
    logger.info(`✅ Validation: ${validatedInsights.length}/${insights.length} valid`, {
      component: 'insightQualityService',
      operation: 'validate',
    });
    ...
    logger.info(
      `✅ Quality filter: ${qualityInsights.length}/${scoredInsights.length} meet threshold (${minimumQuality})`,
      { component: 'insightQualityService', operation: 'filter' }
    );
    ...
    logger.info(`✨ Final output: ${rankedInsights.length} high-quality, unique insights`, {
      component: 'insightQualityService',
      operation: 'complete',
      metadata: {
        avgScore: (
          rankedInsights.reduce((sum, i) => sum + i.qualityScore.overall, 0) /
  ...
  static getQualityTier(score: number): 'excellent' | 'good' | 'fair' | 'poor' {
    ...
    if (score >= this.QUALITY_THRESHOLDS.minimum) return 'fair';
    return 'poor';
  }
  ...
  static generateQualityReport(scoredInsights: ScoredInsight[]): {
    ...
    const avgScores: InsightQualityScore = {
      relevance: 0,
      actionability: 0,
      novelty: 0,
      impact: 0,
    ...
    if (scoredInsights.length > 0) {
      ...
      Object.keys(avgScores).forEach(key => {
        avgScores[key as keyof InsightQualityScore] /= scoredInsights.length;
      });
    }
