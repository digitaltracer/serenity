...
export type QuickAddResult =
  | { kind: 'task'; task: Omit<Task, 'id' | 'createdAt' | 'updatedAt'>; debug?: any }
  | { kind: 'journal'; entry: Omit<JournalEntry, 'id' | 'createdAt' | 'updatedAt'>; debug?: any };

type Priority = 'low' | 'medium' | 'high';

const WEEKDAYS = ['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];

function clampDateToLocalNoon(d: Date) {
  const nd = new Date(d);
...
function parseExplicitDate(s: string): Date | undefined {
  // Try YYYY-MM-DD
  const m1 = s.match(/(\d{4})-(\d{1,2})-(\d{1,2})/);
  if (m1) {
    const d = new Date(Number(m1[1]), Number(m1[2]) - 1, Number(m1[3]));
    if (!isNaN(d.getTime())) return clampDateToLocalNoon(d);
  }
...
function parseRelativeDate(s: string): Date | undefined {
  const text = s.toLowerCase();

  // Today variations
  if (/\b(today|eod|now)\b/.test(text)) return clampDateToLocalNoon(new Date());
...
function detectPriority(s: string): Priority | undefined {
  const text = s.toLowerCase();
...
function extractProject(s: string): string | undefined {
...
function stripControlPhrases(s: string): string {
  let out = s
...
function decideKind(s: string): 'journal' | 'task' {
...
export function parseQuickInput(input: string): QuickAddResult {
