...
class UserProfileService {
     ...
     */
    static identifyPeakHours(tasks) {
     ...
     */
    static calculateCompletionRateHistory(tasks) {
        const history = [];
        // Group tasks by week
     ...
     * Analyze common tags
     */
    static analyzeCommonTags(tasks, journalEntries) {
     ...
     */
    static determineJournalingFrequency(entries) {
        if (entries.length === 0)
            return 'none';
        ...
        if (entriesPerDay >= 0.7)
            return 'daily'; // ~5+ entries per week
        if (entriesPerDay >= 0.2)
            return 'weekly'; // ~1-4 entries per week
        return 'sporadic';
    }
    /**
     * Analyze journaling patterns
     */
    static analyzeJournalingPatterns(entries) {
     ...
     */
    static analyzeMoodDistribution(entries) {
        const moodCounts = {};
        entries.forEach(entry => {
            if (entry.mood) {
     ...
     */
    static detectSentimentTrend(entries) {
        if (entries.length < 5)
            return 'stable';
        // Simple sentiment analysis based on keywords
        const getSentiment = (content) => {
        ...
        const secondAvg = secondHalf.reduce((sum, e) => sum + getSentiment(e.content), 0) / secondHalf.length;
     ...
     */
    static identifyStressIndicators(entries) {
     ...
     */
    static analyzeInsightPreferences(insights, interactions) {
     ...
     */
    static buildProfile(context, existingProfile) {
        logger_1.logger.info('🧠 Building user profile from data', {
        ...
        const currentCompletionRate = context.tasks.length > 0 ? completedTasks.length / context.tasks.length : 0;
        ...
        logger_1.logger.info('✅ User profile built successfully', {
            component: 'userProfileService',
            operation: 'buildProfile',
            metadata: {
     ...
     */
    static getFocusAreasSummary(profile) {
        const parts = [];
