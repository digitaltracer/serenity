...
import { useToast } from '@serenity/ui';
import { Toggle } from '@serenity/ui';
import { AIUsageSummary } from '@serenity/ui';
import { AIOperationHistory } from '@serenity/ui';
import { AIUsageTrendChart } from '@serenity/ui';
import { AIProviderCredentialManager } from '@serenity/ui';
import {
  Settings,
  Palette,
  Bell,
 ...
 * Note: Database configuration is intentionally excluded - it's only available in the desktop app
 */
export function SettingsPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.usage)) {
          dispatch(restoreUsage(data.usage as any));
        }
      }
    } catch (error) {
  ...
  const handleTestCredential = async (provider: 'openai' | 'gemini' | 'anthropic', apiKey: string) => {
    try {
      // For web app, we test via API route
  ...
  const handleAddCredential = async (credentialData: AIProviderCredentialInput) => {
    try {
      ...
      if (response.ok) {
        showSuccess('API Key Saved', `Successfully saved ${credentialData.provider} API key`);
        setIsAddingCredential(false);
        // Refresh credentials
        (dispatch as any)(fetchCredentials(false));
      } else {
        const data = await response.json();
        showError('Save Failed', data.error || 'Failed to save API key');
      }
    } catch (error: any) {
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800">
                  <p className="text-xs text-blue-700 dark:text-blue-300">
                    <strong>Privacy:</strong> Your data is sent to the selected AI provider for analysis. API keys are stored securely in your account.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            {/* Note about database - Web apps use cloud storage */}
