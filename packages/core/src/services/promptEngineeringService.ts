...
export interface PromptContext {
  ...
  previousInsights?: Array<{ type: string; title: string }>;
...
export class PromptEngineeringService {
   ...
   */
  private static readonly INSIGHT_EXAMPLES: FewShotExample[] = [
    {
        ...
        insights: [
          {
            ...
            description:
              'You have 8 overdue high-priority tasks, and your journal entries indicate feeling overwhelmed. This suggests you may have taken on too much. Consider: 1) Review each overdue task and reschedule non-critical ones, 2) Delegate where possible, 3) Focus on completing just 2-3 high-impact tasks this week.',
            confidence: 0.85,
            category: 'tasks',
            actionable: true,
            metadata: {
        ...
        insights: [
          {
            ...
            description:
              'Analysis shows you complete 60% of your tasks between 9-11am, suggesting this is your peak productivity window. Recommendation: Block 9-11am daily for your most important or challenging work. Save meetings and administrative tasks for afternoons.',
            confidence: 0.9,
            category: 'habits',
            actionable: true,
            metadata: {
          ...
          },
          {
            ...
            description:
              '"Design" appears frequently in your tasks and journal. Consider: 1) Creating a dedicated "Design" project to organize this work, 2) Allocating specific time blocks for design tasks, 3) Setting a design-related goal to track progress.',
            confidence: 0.75,
            category: 'habits',
            actionable: true,
            metadata: {
        ...
        insights: [
          {
            ...
            description:
              'Excellent work! You completed 12 tasks this week, a 50% increase from last week. Your journal sentiment is also trending positive. You\'re at 75% toward your weekly goal. Keep this momentum by: 1) Maintaining your current routine, 2) Identifying what changed this week, 3) Celebrating this progress.',
            confidence: 0.95,
            category: 'goals',
            actionable: true,
            metadata: {
   ...
   */
  private static generateSystemPrompt(context?: PromptContext): string {
...
- Follow the exact schema provided below`;
      ...
      if (context.previousAnalyses && context.previousAnalyses.length > 0) {
        ...
        context.previousAnalyses.forEach((analysis, index) => {
          const daysAgo = Math.floor(
            (Date.now() - new Date(analysis.created_at).getTime()) / (1000 * 60 * 60 * 24)
          );
   ...
   */
  private static generateFewShotPrompt(): string {
   ...
   */
  private static generateSchemaPrompt(): string {
    return `
...
JSON SCHEMA (strictly follow this structure):
{
   ...
   */
  private static generateChainOfThoughtPrompt(): string {
    return `
...
Respond with ONLY the JSON object - no code fences, no additional text.`;
  ...
  static optimizePromptTokens(prompt: string, maxTokens: number): string {
    ...
    if (systemPromptEnd === -1) return prompt.substring(0, maxChars);
   ...
   */
  static generateTargetedPrompt(
    type: 'productivity' | 'habits' | 'wellbeing' | 'goals',
    data: {
      tasks: PreprocessedTask[];
      journalEntries: PreprocessedJournalEntry[];
      context?: PromptContext;
