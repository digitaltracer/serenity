...
exports.AICredentialService = void 0;
...
class AICredentialService {
     ...
     */
    static getAvailableCredentials(credentials) {
        const now = Date.now();
        return credentials
            ...
            if (now < cooldown.until) {
                logger_1.logger.debug(`Credential ${cred.id} is in cooldown until ${new Date(cooldown.until).toISOString()}`, {
     ...
     */
    static getNextAvailableCredential(credentials) {
        const available = this.getAvailableCredentials(credentials);
        if (available.length === 0) {
            logger_1.logger.warn('No available credentials found (all disabled or in cooldown)', {
                component: 'AICredentialService',
        ...
        logger_1.logger.info(`Selected credential: ${credential.provider}/${credential.name} (priority: ${credential.priority})`, {
            component: 'AICredentialService',
            operation: 'getNextAvailableCredential'
        });
        return credential;
     ...
     */
    static getCredentialsForProvider(credentials, provider) {
        return credentials
     ...
     */
    static recordSuccess(credentialId) {
        // Clear cooldown on success
        credentialCooldowns.delete(credentialId);
        logger_1.logger.info(`✅ Recorded success for credential: ${credentialId}`, {
            component: 'AICredentialService',
     ...
     */
    static recordError(credentialId, error) {
        ...
        if (!cooldown) {
            ...
            logger_1.logger.warn(`❌ Credential ${credentialId} failed (will retry in 30s): ${error}`, {
                component: 'AICredentialService',
            ...
            cooldown.retryCount++;
            logger_1.logger.warn(`❌ Credential ${credentialId} failed again (retry ${cooldown.retryCount}/${MAX_RETRIES_BEFORE_SKIP}): ${error}`, {
                component: 'AICredentialService',
                operation: 'recordError'
            });
        }
            ...
            cooldown.retryCount++;
            logger_1.logger.error(`❌ Credential ${credentialId} failed ${cooldown.retryCount} times - skipping for 5 minutes: ${error}`, {
                component: 'AICredentialService',
                operation: 'recordError'
            });
        }
    ...
    static clearCooldowns() {
        ...
        logger_1.logger.info('Cleared all credential cooldowns', {
            component: 'AICredentialService',
     ...
     */
    static getCooldownStatus(credentialId) {
        const cooldown = credentialCooldowns.get(credentialId);
        if (!cooldown) {
     ...
     */
    static generateDefaultName(provider, existingCredentials) {
     ...
     */
    static getNextPriority(existingCredentials) {
...
exports.AICredentialService = AICredentialService;
